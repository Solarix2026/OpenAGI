...
function connect(){console.log('[WS] Connecting to',location.host);try{ws=new WebSocket('ws://'+location.host+'/ws');ws.onopen=()=>{console.log('[WS] Connected');setStatus('online');loadHistory();refreshSidebar();loadMemory();loadCapabilities();};ws.onclose=(e)=>{console.log('[WS] Closed',e.code,e.reason);setStatus('offline');if(e.code!==1000)connect();};ws.onerror=(e)=>{console.error('[WS] Error',e);setStatus('offline');};ws.onmessage=(e)=>{console.log('[WS] Message',e.data.slice(0,100));const d=JSON.parse(e.data);if(d.type==='thinking')showThinking();if(d.type==='response')showResponse(d.text);if(d.type==='proactive')addMsg(d.text,'agent',true);if(d.type==='log')addLog(d);if(d.type==='mode')updateModePill(d.mode);};}catch(e){console.error('[WS] Failed to create',e);setStatus('offline');setTimeout(connect,5000);}}
function setStatus(state){const dot=document.getElementById('status-dot'),txt=document.getElementById('status-text');dot.className='status-dot '+(state==='online'?'online':state==='thinking'?'thinking-st':'offline');txt.textContent=state==='online'?'Online':state==='thinking'?'Thinking':state==='offline'?'Disconnected':'Connecting';}
...
function showThinking(){document.getElementById('thinking-row').style.display='flex';thinkStart=Date.now();thinkTimer=setInterval(()=>{const s=((Date.now()-thinkStart)/1000).toFixed(1);document.getElementById('elapsed').textContent=` ${s}s`;},100);const msgs=document.getElementById('messages');requestAnimationFrame(()=>{msgs.scrollTop=msgs.scrollHeight;document.getElementById('thinking-row').scrollIntoView({behavior:'smooth',block:'end'});});setStatus('thinking');}
